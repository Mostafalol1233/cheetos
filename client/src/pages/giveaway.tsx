  ...
  return (
    <div className="min-h-screen relative" style={{ fontFamily: "'Inter',system-ui,sans-serif", color: "#fff" }}>
      ...
      <div className="relative z-10 pt-24">
        ...
        {/* ── GIVEAWAY ENDED NOTICE ── always visible ── */}
        <div className="max-w-2xl mx-auto px-5 pt-8 pb-2">
          <div className="rounded-2xl border border-white/8 bg-black/55 backdrop-blur-sm p-6 text-center"
            ...
            <p className="text-xs font-black uppercase tracking-[0.25em] text-white/30 mb-1">
              {lang === "ar" ? "انتهت المسابقة" : "Giveaway Ended"}
            </p>
            <h2 className="text-xl font-black text-white mb-1">
              {lang === "ar" ? "شكراً لجميع المشاركين" : "Thank You to All Participants"}
            </h2>
            <p className="text-white/40 text-sm mb-6">
              {lang === "ar"
                ? "انتهى السحب رسمياً. تابعونا على واتساب ويوتيوب للمسابقات القادمة."
                : "The draw is officially closed. Follow us on WhatsApp & YouTube for upcoming giveaways."}
            </p>
            <div className="flex gap-3 justify-center flex-wrap">
              <a href={cfg.wa_url} target="_blank" rel="noopener noreferrer"
                ...
                {lang === "ar" ? "قناة الواتساب" : "WhatsApp Channel"}
              </a>
              <a href={cfg.yt_url} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all"
                style={{ background: "rgba(255,0,0,0.10)", border: "1px solid rgba(255,0,0,0.25)", color: "#ff4444" }}
                onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = "rgba(255,0,0,0.20)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = "rgba(255,0,0,0.10)"; }}>
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
