...
(async () => {
  try {
    ...
    if (gameRes.rows.length > 0) {
      ...
      // Reset discount_price to null for all packages
      await pool.query('UPDATE game_packages SET discount_price = NULL WHERE game_id = $1', [gameId]);
      console.log('Reset discount prices for Crossfire packages');
